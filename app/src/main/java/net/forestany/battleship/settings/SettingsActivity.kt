package net.forestany.battleship.settings
...
class SettingsActivity : AppCompatActivity() {
    ...
    class SettingsFragment(private val context: Context, private val view: View, private val anchorView: View?) : PreferenceFragmentCompat(), OnSharedPreferenceChangeListener {
            ...
            return when (preference.key) {
                "grid_cell_size" -> {
                    SliderDialogFragment.newInstance(
                        prefKey = "grid_cell_size",
                        title = getString(R.string.settings_general_grid_cell_size),
                        min = 30,
                        max = 60,
                        defaultValue = 32
                ...
                "grid_cell_padding" -> {
                    SliderDialogFragment.newInstance(
                        prefKey = "grid_cell_padding",
                        title = getString(R.string.settings_general_grid_cell_padding),
                        min = 0,
                        max = 10,
                        defaultValue = 2
        ...
        override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences?, key: String?) {
            ...
            if (key.contentEquals("general_locale")) {
            ...
            } else if (key.contentEquals("grid_cell_size")) {
                // not empty, isInt, between 30 and 60
                if (value.toString().isEmpty()) {
                    ...
                    wrongValue = true
                } else if (Integer.parseInt(value.toString()) < 30) {
                    exceptionMessage = getString(R.string.settings_value_to_low, 30)
                    wrongValue = true
                } else if (Integer.parseInt(value.toString()) > 60) {
                    exceptionMessage = getString(R.string.settings_value_to_high, 60)
                    wrongValue = true
                }
            } else if (key.contentEquals("grid_cell_padding")) {
                // not empty, isInt, between 0 and 10
                if (value.toString().isEmpty()) {
                    exceptionMessage = getString(R.string.settings_value_empty)
                    wrongValue = true
                } else if (!net.forestany.forestj.lib.Helper.isInteger(value.toString())) {
                    ...
                    exceptionMessage = getString(R.string.settings_value_to_high, 10)
                    wrongValue = true
                }
